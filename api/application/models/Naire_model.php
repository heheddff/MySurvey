...
class Naire_model extends CI_Model
{
    ...
    public function get_naires()
    {
        ...
        $options = $this->db->query("select * from options where options.n_id = {$n_id}")
            ->result_array();
        $rows = $this->db->query("select * from rows where rows.n_id = {$n_id}")
            ->result_array();
        $columns = $this->db->query("select * from columns where columns.n_id = {$n_id}")
            ->result_array();
...
//		echo var_dump($options);
        ...
        foreach ($questions as $questionkey => $questionval) {
            ...
            $rowstemp = [];
            foreach ($rows as $optionitem => $optionval) {
                if ($questionval['q_id'] == $optionval['q_id']) {
                    $rowstemp[] = array(
                        "r_id" => $optionval['r_id'],
                        "content" => $optionval['r_value'],
                        "isAddition" => $optionval['r_isaddtion'] == "1" ? true : false
                    );
                }
            }
            
            $columnstemp = [];
            foreach ($columns as $optionitem => $optionval) {
                if ($questionval['q_id'] == $optionval['q_id']) {
                    $columnstemp[] = array(
                        "c_id" => $optionval['c_id'],
                        "content" => $optionval['c_value'],
                        "isAddition" => $optionval['c_isaddtion'] == "1" ? true : false
                    );
                }
            }
            ...
            /*if ($questionval["q_type"] == '1' or $questionval["q_type"] == '12') {
                $result['topic'][] = array(
                    "q_id" => $questionval["q_id"],
                    "question" => $questionval["q_content"],
                    "isRequired" => $questionval["q_isrequire"] == "1" ? true : false,
    ...
    private function check_option($value,$type=1){
        if($type==1){
            if ($value['deadline'] === '' || $value['title'] === '' || $value['status'] === '') {
                    return array("err" => 1, "data" => "问卷(naire)必填字段不能为空");
                }
        }
        elseif ($type == 2) {
            if ($value['question'] === '' || $value['type'] === '' || $value['isRequired'] === '') {
        ...
        if ($status == 'create') {
            ...
            try{
                ...
                foreach ($naire['topic'] as $topickey => $topicval) {
                    $this->check_option($topicval, $type=2);
                    // 题目内容
                    ...
                    if (!empty($topicval['options']) && $topicval['type'] != 4 && $topicval['type'] != 6) {
                        // 遍历选项
                    ...
                    // 行标题
                    if (isset($topicval['rows']) && count($topicval['rows'])>0) {
                        // 遍历选项
                        foreach ($topicval['rows'] as $optionkey => $optionval) {
                            // 选项内容 $optionval['content']
                            // 选项是否需要填写附加内容 $optionval['isAddition']
                            if ($optionval === '') {
                                return array("err" => 1, "data" => '行字段不能为空');
                            }
                            $insert_option_data = array(
                                'r_value' => trim($optionval['content']),
                                'n_id' => $naire_id,
                                'q_id' => $question_id,
                                //'r_isaddtion' => $optionval['isAddition'] == "true" ? 1 : 0
                                'r_isaddtion' => 0
                            );
                            $this->db->insert('rows', $insert_option_data);
                            //print_r($optionval['isAddition'] == 1 ? 'true' : 'false');
                        }
                    }

                    if (isset($topicval['columns']) && count($topicval['columns']>0)) {
                        // 遍历选项
                        foreach ($topicval['columns'] as $optionkey => $optionval) {
                            // 选项内容 $optionval['content']
                            // 选项是否需要填写附加内容 $optionval['isAddition']
                            if ($optionval === '') {
                                return array("err" => 1, "data" => '列字段不能为空');
                            }
                            $insert_option_data = array(
                                'c_value' => trim($optionval['content']),
                                'n_id' => $naire_id,
                                'q_id' => $question_id,
                                //'r_isaddtion' => $optionval['isAddition'] == "true" ? 1 : 0
                                'c_isaddtion' => 0
                            );
                            $this->db->insert('columns', $insert_option_data);
                            //print_r($optionval['isAddition'] == 1 ? 'true' : 'false');
                        }
                    }
